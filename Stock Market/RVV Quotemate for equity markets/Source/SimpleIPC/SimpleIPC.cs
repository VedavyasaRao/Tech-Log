...
namespace SimpleIPC
{
    public enum SIPCArgsIndex :int { Name, Exception, Returnval, Args };
    public enum SIPCEncoding : byte { binary, json };
    ...
    public sealed class MemoryManager 
    {
        ...
        private unsafe byte* data_with_offset()
        {
        ...
        public void CreateNew(SIPCEncoding encoding)
        {
            bool b;
        ...
        public void StoreParameters(string name, object[] paramlist)
        {
            ...
            unsafe
            {
                UnmanagedMemoryStream memStream = new UnmanagedMemoryStream(data_with_offset(), BUFFER_SIZE-1, BUFFER_SIZE-1, FileAccess.ReadWrite);

                if (encoding == SIPCEncoding.json)
                {
                    var s = JSONPersister<object[]>.GetJSON(data);
                    var utf8 = new UTF8Encoding();
                    byte[] bytebuf = utf8.GetBytes(s);

                    int len = bytebuf.Length;
                    byte[] intBytes = BitConverter.GetBytes(len);
                    memStream.Write(intBytes, 0, intBytes.Length);
                    memStream.Write(bytebuf, 0, bytebuf.Length);
                }
                else
                {
        ...
        public object[] RetriveParameters()
        {
            ...
            unsafe
            {
                ...
                if (encoding == SIPCEncoding.json)
                {
                    ...
                    memStream.Read(intBytes, 0, len);
                    len = BitConverter.ToInt32(intBytes, 0);

                    byte[] bytebuf = new byte[len];
                    memStream.Read(bytebuf, 0, len);

                    var utf8 = new UTF8Encoding();
                    var s = utf8.GetString(bytebuf);
                    data = JSONPersister<object[]>.SetJSON(s);
                }
                else
                {
                    BinaryFormatter formatter = new BinaryFormatter();
        ...
        public void StoreResults(object[] returnlist)
        {
            unsafe
            {
                ...
                try
                {
                    if (encoding == SIPCEncoding.json)
                    {
                        var ex = returnlist[(int)SIPCArgsIndex.Exception];
                        if (ex != null)
                        {
                            returnlist[(int)SIPCArgsIndex.Exception] = GetFullExceptionDetails(ex as Exception);
                        }
                        var s = JSONPersister<object[]>.GetJSON(returnlist);
                        var utf8 = new UTF8Encoding();
                        byte[] bytebuf = utf8.GetBytes(s);
    ...
    public class SIPCServerImpl 
    {
        ...
        internal SIPCServerImpl(string uniquename, object serverinstance, SIPCEncoding encoding)
        {
            _uniquename = uniquename;
            mm = new MemoryManager(_uniquename);
            mm.CreateNew(encoding);
    ...
    public class GenericProxy<T> : RealProxy, IDisposable
    {
        ...
        public GenericProxy(ProxyBase provider, bool basynccall = false, bool basynccallback = false, bool bevents = false, SIPCEncoding encoding= SIPCEncoding.binary)
            : base(typeof(T))
        {
            ...
            if (bevents)
            {
                ...
                eventserver = new NamedObject.SIPCServer(servername3, eventmgr, encoding);
                eventserver.Start();
            }
    ...
    namespace Windows
    {
        ...
        public class SIPCServer : SingleActionServer
        {
            ...
            public SIPCServer(string uniquename, object serverinstance, SIPCEncoding encoding)
                : base(uniquename, null)
            {
                serverimpl = new SIPCServerImpl(uniquename, serverinstance, encoding);
