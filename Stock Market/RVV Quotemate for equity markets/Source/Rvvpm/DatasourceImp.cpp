...
{
	wstring servername = L"YahooFinanceServer";
	string sbuf;
	m_pimDataSrcs->GetPrivateProfileString("Settings", "Current", "", sbuf);
	if (sbuf == "TD_WATHERHOUSE")
		servername = L"CharlesSchwabServer";
	pquotes.reset(new GetQuotes(servername));
	pnews.reset(new GetNews());
}
...
void CYahooSource::QueryQuotes(CPMScrip* pscrip)
{
	...
	strcpy(readbuf, pquotes->QueryQuote(pscrip->m_symbol).c_str());
	if (readbuf[0] == 0)
		return;
...
void CYahooSource::QueryClose(CPMScrip* pscrip)
{
	...
	try
	{
		strcpy(readbuf, pquotes->QueryCloses(pscrip->m_symbol).c_str());
		if (readbuf[0] == 0)
			return;
	}
	catch (...)
...
void CYahooSource::QueryNews(CPMScrip* pscrip)
{
	...
	ZeroMemory(readbuf, BUFSIZE);
	strcpy(readbuf, pnews->QueryNews(szbuf).c_str());
	if (readbuf[0] == 0)
		return;
...
void CYahooSource::QueryIndices(CString& dow, CString& nasdaq)
{
	...
	char szbuf[100];
	ZeroMemory(readbuf, BUFSIZE);
	strcpy(readbuf, pquotes->QueryIndicies().c_str());
	if (readbuf[0] == 0)
		return;
...
void CYahooSource::QueryDateTime(CString& datetime)
{
	char szbuf[100];
	ZeroMemory(readbuf, BUFSIZE);
	strcpy(readbuf, pquotes->QueryDateTime().c_str());
	if (readbuf[0] == 0)
		return;
