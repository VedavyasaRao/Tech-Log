...
BOOL CRVVPMApp::InitInstance()
{
	...
	string sbuf;
	m_imDataSrcs.GetPrivateProfileString("Settings", "Current", "", sbuf);
	m_szYahooFinance = m_szOutput + CString("\\YahooFinance\\YahooFinance.exe");
	if (sbuf == "TD_WATHERHOUSE")
		m_szYahooFinance = m_szOutput + CString("\\CharlesSchwab\\CharlesSchwab.exe");
	...
	auto pdatasrc = new CYahooSource();
	m_pquotesrc = pdatasrc;
	m_pclosesrc = pdatasrc;
	m_pnewssrc = pdatasrc;
...
void CRVVPMApp::StartYahooFinance()
{
	...
	if (sbuf == "TD_WATHERHOUSE")
	{
		sprintf(Args, "%s t", m_szYahooFinance);
		...
		if (!CreateProcess(NULL, Args, NULL, NULL, FALSE,
			0,
			NULL,
			m_szOutput + CString("\\CharlesSchwab"),
			&StartupInfo,
			&m_yahooproc))
		{
			return;
	...
	sprintf(Args, "%s r", m_szYahooFinance);
	ZeroMemory(&StartupInfo, sizeof(StartupInfo));
	ZeroMemory(&m_yahooproc, sizeof(m_yahooproc));
	StartupInfo.cb = sizeof(STARTUPINFO);
	...
	if (!CreateProcess(NULL, Args, NULL, NULL, FALSE,
		...
		NULL,
		m_szOutput + ((sbuf == "TD_WATHERHOUSE")? CString("\\CharlesSchwab"):CString("\\YahooFinance")),
		&StartupInfo,
		&m_yahooproc))
	{
		return;
...
void	CRVVPMApp::StopYahooFinanceGoogleNews()
{
	...
	WaitForSingleObject(m_gnewsproc.hProcess, 5000);


	WinExec("taskkill /f /t /im yahoofinance.exe", SW_HIDE);
	WaitForSingleObject(m_yahooproc.hProcess, 5000);

	WinExec("taskkill /f /t /im CharlesSchwab.exe", SW_HIDE);
	WaitForSingleObject(m_yahooproc.hProcess, 5000);
...
void CRVVPMApp::LoadNews()
{
	...
	for (int i=0; i<m_currentlist.GetSize(); ++i)
	{
		...
		m_pnewssrc->QueryNews(m_currentlist[i]);
		m_currentlist[i]->Calucalate();
		if (m_currentlist[i]->m_alramflag)
			alramflag = 1;
	}
