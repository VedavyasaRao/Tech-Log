...
cmd = """" & objShell.ExpandEnvironmentStrings( "%INSTALLDIR%" ) & "\setup\scripts\DComPerm.exe" & """" & " -ma set " & """" & "NT AUTHORITY\ANONYMOUS LOGON" & """" & "  permit level:l,r"
...
if left(strosver,1) <> "6" then
    ...
    cmd="netsh firewall add allowedprogram  " & """" & "C:\Program Files\TAF\TAF.exe" & """" & " TAF enable subnet"
    objShell.Run "c:\windows\system32\cmd /c" & """" & cmd & """" ,0,True

    cmd="netsh firewall add allowedprogram  "  & """" &  "C:\Program Files\TAF\TAFControllerService.exe"  & """" &  " TAFControllerService enable subnet"
    objShell.Run "c:\windows\system32\cmd /c" & """" & cmd & """" ,0,True
    ...
    cmd="netsh firewall add allowedprogram  " & """" & "C:\Program Files (x86)\TAF\TAF.exe" & """" & " TAF enable subnet"
    objShell.Run "c:\windows\system32\cmd /c" & """" & cmd & """" ,0,True

    cmd="netsh firewall add allowedprogram  "  & """" &  "C:\Program Files (x86)\TAF\TAFControllerService.exe"  & """" &  " TAFControllerService enable subnet"
    objShell.Run "c:\windows\system32\cmd /c" & """" & cmd & """" ,0,True
