...
namespace Incremental_Zip
{
    class PipeServer
    {
        ...
        public static void driver(object src, string[] args, out ArchiveInfo tgt)
        {
            ...
            var pipename = @"\\.\pipe\brinfoK";

            if (iszipfile(fullpath, passwd))
            {
                ...
                if (args[0] == "a")
                {
                    ...
                    string cmdline = $"/c \"{App.sevenzpath}\" a -p{passwd} -si -mhe=on  {fname} < {pipename}";
                    Process.Start("cmd", cmdline).WaitForExit();
                    t.Join();
                    File.Copy(fname, fullpath, true);
                    File.Delete(fname);
                }
                ...
                else if (args[0] == "e")
                {
                    ...
                    t.Start();
                    System.Threading.Thread.Sleep(2000);
                    File.Copy(fullpath, fname, true);
                    string cmdline = $"/c \"{App.sevenzpath}\" e -p{passwd} -so  {fname} > {pipename}";
                    Process.Start("cmd", cmdline).WaitForExit();
                    t.Join();
                    File.Delete(fname);
                    tgt = ret;
            ...
            {
                if (args[0] == "a")
                    FileEncrypt(src, fullpath, passwd);
                else if (args[0] == "e")
                    tgt = (ArchiveInfo)FileDecrypt(fullpath, passwd);
            }
        }
    }
}
